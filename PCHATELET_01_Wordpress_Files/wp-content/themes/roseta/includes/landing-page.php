...
function roseta_lpblocks( $sid = 1 ) {
	...
	?>
	<section id="lp-blocks<?php echo $sid ?>" class="lp-blocks lp-blocks<?php echo $sid ?> lp-blocks-rows-<?php echo apply_filters('roseta_blocks_perrow', $pagecount, $sid) ?>">
    ...
    if ( $custom_query->have_posts() ) : ?>
		<section id="lp-boxes-<?php echo absint( $sid ) ?>" class="lp-boxes lp-boxes-<?php echo absint( $sid ) ?> <?php  echo esc_attr( $animated_class ) ?> lp-boxes-rows-<?php echo absint( $options['theme_lpboxrow' . $sid] ); ?>">
    		...
    		<?php while ( $custom_query->have_posts() ) :
		            ...
		            $box['content'] = $excerpt;
		            list( $box['image'], ) = wp_get_attachment_image_src( get_post_thumbnail_id( get_the_ID() ), 'roseta-lpbox-' . $sid );
...
function roseta_lptext_output( $data ){ ?>
	<section class="lp-text<?php echo ( ! empty( $data['image'] ) ) ? ' lp-text-hasimage': '' ?>" id="lp-text-<?php echo esc_attr( $data['id'] ); ?>" >
